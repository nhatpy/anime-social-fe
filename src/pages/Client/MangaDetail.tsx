...
export const MangaDetail = () => {
    const items = [{title: <Link to="/">Trang chủ</Link>}, {title: <Link to="/search">Thể loại</Link>}, {title: <p>Chuyển sinh thành liễu đột biến</p>}]
                ...
                <div className="flex flex-row w-full gap-4">
                    <div className="flex flex-col w-full gap-2">
                        <div className="flex flex-col w-full h-fit-content justify-center items-center">
                            <h3 className="text-2xl text-[22px] uppercase">chuyển sinh thành liễu đột biến</h3>
                            <p className="text-sm italic text-gray-500">[Cập nhật lúc: 20/02/2025 22:31:06]</p>
                        </div>
                        <div className="flex flex-row w-full justify-between">
                            <div className="flex flex-col w-[29%] gap-2">
                                <img src="/assets/images.jpg" alt="" className="w-[250px] h-[300px]" />
                            </div>
                            <div className="flex flex-col w-[69%] gap-2 text-gray-500 text-lg">
                                <div className="flex flex-row gap-2 w-full">
                                    <p className="flex w-[30%] items-center h-fit gap-1">{icons.user}Tác giả</p>
                                    <p className="w-[70%] float">Long Nhật</p>
                                </div>
                                <div className="flex flex-row gap-2 w-full">
                                    <p className="flex w-[30%] items-center h-fit gap-1">{icons.status}Tình trạng</p>
                                    <p className="w-[70%] float">Đang tiến hành</p>
                                </div>
                                <div className="flex flex-row gap-2 w-full">
                                    <p className="flex w-[30%] items-center h-fit gap-1">{icons.tag}Thể loại</p>
                                    <p className="w-[70%] float">
                                        {["Action", "Adventure", "Comedy", "Fantasy", "Shounen"].map((item, index, array) => (
                                ...
                                <div className="flex flex-row gap-2 w-full">
                                    <p className="flex w-[30%] items-center h-fit gap-1">{icons.eye}Lượt xem</p>
                                    <p className="w-[70%] float">100K</p>
                                </div>
                                <div className="flex flex-row gap-2 w-full">
                                    <p className="flex w-[30%] items-center h-fit gap-1">{icons.description}Mô tả</p>
                                    <p className="w-[70%] float">Ngu Tử Du bị chết sặc rồi chuyển sinh thành một cây liễu đột biến và bắt đầu cuộc phiêu lưu của mình.</p>
                                </div>
                                <div className="flex flex-row w-full gap-5">
                                    ...
                                    <p><span className="font-bold">190.403</span> người đang theo dõi</p>
                                </div>
                        ...
                        <div className="border rounded-lg p-4 bg-white w-full mx-auto">
                            <div className="flex items-center border-b pb-2 mb-2">
                                <span className="font-semibold flex items-center gap-2">{icons.book} Danh sách chương</span>
                            </div>
                            <table className="w-full border-collapse">
                                ...
                                {chaptersData.slice(0, visibleChapters).map((chapter) => (
                                    ...
                                    >
                                        <Link to={"/manga/chuyen-sinh-thanh-lieu-dot-bien/chapter-12"}>{chapter.title}</Link>
                                    </td>
                                    <td className="p-2 text-gray-500 italic">{chapter.timeAgo}</td>
                                    </tr>
                                ))}
                                </tbody>
                            </table>
